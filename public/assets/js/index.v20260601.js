    (() => {
      ...
      const PERFORMANCE_LOG_PREFIX = '[den-perf]';
      const JS_BOOT_START_MS = performance.now();
      ...
      const DISCORD_WIDGET_URL = DISCORD_GUILD_ID
        ...
        : null;
      const DISCORD_BUTTON_IMAGE_URL = 'assets/images/join_disc_blue.png';
      ...
      const CALENDAR_HTML_URL = '/calendar.html';
      ...
      const SCENE_TILE_IMAGE_SOURCES = Object.freeze([
        Object.freeze({
          avif: 'assets/images/den_arcade.v20260601.avif',
          webp: 'assets/images/den_arcade.v20260601.webp',
          fallback: 'assets/images/den_arcade.v20260528.PNG'
        }),
        Object.freeze({
          avif: 'assets/images/den_computer.v20260601.avif',
          webp: 'assets/images/den_computer.v20260601.webp',
          fallback: 'assets/images/den_computer.png'
        }),
        Object.freeze({
          avif: 'assets/images/den_chapel.v20260601.avif',
          webp: 'assets/images/den_chapel.v20260601.webp',
          fallback: 'assets/images/den_chapel.v20260528.png'
        })
      ]);
      const SCENE_TILE_EAGER_INDEX = 1;
      ...
      function emitPerfMetric(name, value) {
        const metricValue = Number.isFinite(value) ? Math.round(value * 100) / 100 : null;
        if (!Number.isFinite(metricValue)) return;
        perfMetrics[name] = metricValue;
        try {
          window.dispatchEvent(new CustomEvent('den:perf-metric', { detail: { name, value: metricValue } }));
        } catch (_) {}
      }

      function setupPerformanceGates() {
        try {
          const navEntry = performance.getEntriesByType('navigation')[0];
          if (navEntry && Number.isFinite(navEntry.responseStart)) {
            emitPerfMetric('ttfbMs', navEntry.responseStart);
          }
        } catch (_) {}
        ...
        try {
          const lcpObserver = new PerformanceObserver((entryList) => {
            ...
            if (latestEntry && Number.isFinite(latestEntry.startTime)) {
              emitPerfMetric('lcpMs', latestEntry.startTime);
            }
          });
          ...
          const inpObserver = new PerformanceObserver((entryList) => {
            for (const entry of entryList.getEntries()) {
              if (Number.isFinite(entry.duration)) {
                maxInpDuration = Math.max(maxInpDuration, entry.duration);
              }
            }
            emitPerfMetric('inpMs', maxInpDuration);
          });
        ...
        window.addEventListener('load', () => {
          window.setTimeout(() => {
            console.info(PERFORMANCE_LOG_PREFIX, perfMetrics);
          }, 0);
        }, { once: true });
      }

      setupPerformanceGates();
      ...
      function syncBigTvContentVisibility() {
        ...
        const shouldShowBigTvOverlay = aquariumActive || nedryGateActive || promptActive || toolsActive || loginActive || calendarActive;
      ...
      function showCalendarBigTvOverlay() {
        ...
        if (calendarIframeEl && !calendarIframeEl.getAttribute('src')) {
          calendarIframeEl.src = CALENDAR_HTML_URL;
        }
      ...
      function getZeldaSecretAudioElement() {
        if (!zeldaSecretAudioEl) {
          ...
          zeldaSecretAudioEl.preload = 'none';
        }
      ...
      function markSceneReady() {
        ...
        emitPerfMetric('jsBootMs', performance.now() - JS_BOOT_START_MS);
      }
      ...
      function triggerCommodorePowerOnSequence() {
        if (isCommodorePoweringOn) {
          ...
          [commodoreShadowOverlayEl, bigTvShadowOverlayEl, leftMonitorShadowOverlayEl, rightMonitorShadowOverlayEl].forEach(animateMonitorShadowOff);
          return;
        }
        ...
        [bigTvShadowOverlayEl, leftMonitorShadowOverlayEl, rightMonitorShadowOverlayEl].forEach((el) => {
          if (!el) return;
      ...
      function supportsImageType(type) {
        try {
      ...
      function resolveSceneTileSource(tileSource) {
        if (supportsImageType('image/avif')) {
          return tileSource.avif;
        }
        if (supportsImageType('image/webp')) {
          return tileSource.webp;
        }
        return tileSource.fallback;
      }

      function createSceneTiles() {
        sceneLayer.textContent = '';
        SCENE_TILE_IMAGE_SOURCES.forEach((sourceSet, index) => {
          const tile = document.createElement('img');
          tile.className = 'scene-tile';
          tile.src = resolveSceneTileSource(sourceSet);
          tile.alt = '';
          tile.loading = index === SCENE_TILE_EAGER_INDEX ? 'eager' : 'lazy';
          tile.fetchPriority = index === SCENE_TILE_EAGER_INDEX ? 'high' : 'auto';
          tile.decoding = 'async';
      ...
      function createOverlays() {
        ...
        overlayDefaults.forEach((overlay) => {
          ...
          if (overlay.id === AQUARIUM_OVERLAY_ID) {
            ...
            calendarBigTvOverlayEl.setAttribute('aria-hidden', 'true');
            calendarBigTvOverlayEl.setAttribute('aria-label', 'Open Calendar');
            calendarBigTvOverlayEl.title = 'Open Calendar';
            calendarBigTvOverlayEl.addEventListener('pointerdown', (event) => event.stopPropagation());
            calendarBigTvOverlayEl.addEventListener('click', (event) => {
              event.stopPropagation();
              window.location.assign(CALENDAR_HTML_URL);
            });
            calendarIframeEl = document.createElement('iframe');
            calendarIframeEl.className = 'calendar-big-tv-iframe';
            calendarIframeEl.title = 'Calendar preview';
            calendarIframeEl.setAttribute('aria-hidden', 'true');
            calendarBigTvOverlayEl.appendChild(calendarIframeEl);
      ...
      async function initializeScene() {
        ...
        if (saveResultFlash) {
          ...
          debugStatus.textContent = 'Loading server hotspots...';
        }

        void loadAquariumShrimpClipCatalog();
        void fetchDiscordAuthState();
      ...
      void initializeScene().finally(markSceneReady);
    })();
